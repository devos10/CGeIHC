...
float  ciclo1 = 0, ciclo3 = 0, ciclo4 = 0, ciclo5 = 0, ciclo6 = 0;
float ciclo7 = 0, ciclo8 = 0, ciclo9 = 0, ciclo10 = 0, ciclo11 = 0;
Window mainWindow;
...
int i_max_steps = 300; //Número de pasos entre cuadros para interpolación, a mayor número , más lento será el movimiento
int i_curr_steps = 0;
...
void saveFrame(void) //tecla L - MODIFICADA para guardar automáticamente
{
	printf("frameindex %d\n", FrameIndex);
	...
	FILE* archivo = NULL;
	errno_t err = fopen_s(&archivo, "keyframes.txt", "a");

	if (err == 0 && archivo != NULL)
	{
		// Guardar TODOS los frames (desde 0 hasta el actual)
		...
		fclose(archivo);
		printf("Frame guardado en archivo (Total: %d frames)\n", FrameIndex);
	}
	else
	{
...
int main()
{
	...
	// CARGAR KEYFRAMES DESDE ARCHIVO (si existe)
	...
	printf("L       - Guardar frame actual (se guarda automaticamente en archivo)\n");
	printf("P       - Habilitar guardar nuevo frame\n");
	printf("Posicion en X (movAvion_x):\n");
	printf("  1             - Incrementar movAvion_x (+1.0)\n");
	printf("  2             - Habilitar modificacion de movAvion_x\n\n");
	printf("Posicion en Y (movAvion_y):\n");
	printf("  3             - Incrementar movAvion_y (+1.0)\n");
	printf("  4             - Habilitar modificacion de movAvion_y\n");
	printf("  5             - Decrementar movAvion_y (-1.0)\n");
	printf("  6             - Habilitar modificacion de movAvion_y\n\n");
	printf("Rotación (giroAvion):\n");
	printf("  7             - Incrementar giroAvion (+45.0 grados)\n");
	printf("  0             - Habilitar modificacion de giroAvion (tecla 7)\n");
	printf("  8             - Decrementar giroAvion (-45.0 grados)\n");
	printf("  9             - Habilitar modificacion de giroAvion (tecla 8)\n\n");
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_0])
	{
		...
		else if (ciclo10 < 1 && reproduciranimacion == 0) {
			ciclo7 = 0;
			ciclo10++;
	...
	if (keys[GLFW_KEY_7])
	{
		if (ciclo7 < 1) {
			...
			printf("\n giroAvion es: %f\n", giroAvion);
			ciclo7++;
			ciclo10 = 0;  // Cambiado para habilitar con tecla 0 (cero)
	...
	//Habilitar modificación de giroAvion (tecla 8)
	if (keys[GLFW_KEY_9])
	{
		if (ciclo9 < 1) {
			ciclo8 = 0;
			ciclo9++;
			printf("\n Ya puedes modificar tu variable presionando la tecla 8\n");
		}
	}
