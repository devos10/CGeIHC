...
//variables para keyframes de reloj	
float ciclohora = 0, ciclominuto = 0;
...
float giroAvion = 0;
//para el reloj
float manecilla_hora = 0, manecilla_minuto=0;

#define MAX_FRAMES 100 //Número de cuadros máximos
int i_max_steps = 500; //Número de pasos entre cuadros para interpolación, a mayor número , más lento será el movimiento
int i_curr_steps = 0;
typedef struct _frame
{
	...
	float manecilla_hora;
	float manecilla_minuto;
	float manecilla_horaInc;
	float manecilla_minutoInc;
}FRAME;
...
void loadFramesFromFile() {
	FILE* archivo = NULL;
	errno_t err = fopen_s(&archivo, "keyframes.txt", "r");

	if (err != 0 || archivo == NULL) {
		printf("No se pudo abrir keyframes.txt, iniciando sin keyframes.\n");
		FrameIndex = 0;
		return;
	}
	...
	while (!feof(archivo) && FrameIndex < MAX_FRAMES) {
		float hora, minuto;
		if (fscanf(archivo, "%f,%f\n", &hora, &minuto) == 2) {
			KeyFrame[FrameIndex].manecilla_hora = hora;
			KeyFrame[FrameIndex].manecilla_minuto = minuto;
			FrameIndex++;
		}
	}

	fclose(archivo);
	printf("Se cargaron %d keyframes desde archivo.\n", FrameIndex);
}
...
void saveFrame(void) //tecla L - MODIFICADA para guardar automáticamente
{
	...
	KeyFrame[FrameIndex].manecilla_hora = manecilla_hora;
	KeyFrame[FrameIndex].manecilla_minuto = manecilla_minuto;
	...
	if (err == 0 && archivo != NULL)
	{
		for (int i = 0; i < FrameIndex; i++)
		{
			fprintf(archivo, "%.2f,%.2f\n",
				KeyFrame[i].manecilla_hora,
				KeyFrame[i].manecilla_minuto);
...
void resetElements(void) //Tecla 0
{
	...
	manecilla_hora = KeyFrame[0].manecilla_hora;
	manecilla_minuto = KeyFrame[0].manecilla_minuto;
}
...
void interpolation(void)
{
	...
	*/
	KeyFrame[playIndex].manecilla_horaInc = (KeyFrame[playIndex + 1].manecilla_hora - KeyFrame[playIndex].manecilla_hora) / i_max_steps;
	KeyFrame[playIndex].manecilla_minutoInc = (KeyFrame[playIndex + 1].manecilla_minuto - KeyFrame[playIndex].manecilla_minuto) / i_max_steps;
}
...
void animate(void)
{
	...
	if (play)
	{
		...
		else
		{
			...
			manecilla_hora += KeyFrame[playIndex].manecilla_horaInc;
			manecilla_minuto += KeyFrame[playIndex].manecilla_minutoInc;
			i_curr_steps++;
		}
...
int main()
{
	...
	printf("Control de manecilla de minutos\n");
	printf("M       - mueve manecilla de minutos\n");
	printf("N       - Habilitar tecla M de nuevo\n\n");
	printf("Control de manecilla de horas\n");
	printf("H       - mueve manecilla de horas (-30 grados)\n");
	printf("J       - Habilitar tecla H de nuevo\n\n");
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_L])
	{
		...
		{
			saveFrame();
			printf("manecilla_minuto es: %f\n", manecilla_minuto);
			printf("manecilla_hora es: %f\n", manecilla_hora);
	...
	//teclas para manecillas de minutos
	if (keys[GLFW_KEY_M]) {
		if (ciclominuto < 1) {
			printf("\n manecilla minuto es: %f\n", manecilla_minuto);
			manecilla_minuto -= 10.0f;
			printf("\n manecilla minuto es: %f\n", manecilla_minuto);
			ciclominuto++;
			ciclo1 = 0;
	...
	if (keys[GLFW_KEY_N]) {
		if (ciclo1 < 1) {
			ciclominuto = 0;
			ciclo1++;
			printf("\n Ya puedes modificar la variable presionando la tecla M\n");
		}
	}
	//teclas para las manecillas de horas
	if (keys[GLFW_KEY_H]) {
		if (ciclohora < 1) {
			printf("\n manecilla minuto es: %f\n", manecilla_minuto);
			manecilla_hora -= 30.0f;
			printf("\n manecilla hora es: %f\n", manecilla_hora);
			ciclohora++;
			ciclo2 = 0;
	...
	if (keys[GLFW_KEY_J]) {
		if (ciclo2 < 1) {
			ciclohora = 0;
			ciclo2++;
			printf("\n Ya puedes modificar la variable presionando la tecla H\n");
		}
	}
