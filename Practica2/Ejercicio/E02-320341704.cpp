...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// cuadrado rojo -> rectángulo
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.0f, -0.40f, -4.0f));
		model = glm::scale(model, glm::vec3(0.85f, 1.10f, 1.0f));   // ancho, alto
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshColorList[6]->RenderMeshColor(); // cuadrado rojo
		...
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(-0.20f, -0.10f, -3.0f));
		...
		// Puerta cuadrado verde lo pase a rectangulo
		model = glm::mat4(1.0f);
