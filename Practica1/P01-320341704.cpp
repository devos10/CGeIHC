...
#include <glew.h>
#include <glfw3.h>
#include <stdlib.h> //de aqui obtenemos rand
#include <time.h> //da acceso a funciones relacionadas con la fecha y hora
...
int main()
{
	...
	//CREAR VENTANA
	GLFWwindow *mainWindow = glfwCreateWindow(WIDTH, HEIGHT, "Primer ventana - Práctica1", NULL, NULL);
 ...
 //Llamada a las funciones creadas antes del main
	...
	srand(time(NULL));  // semilla basada en la hora actual
	//para los colores de la pantalla sean random
	float uno = (float)rand() / RAND_MAX;
	float dos = (float)rand() / RAND_MAX;
	float tres = (float)rand() / RAND_MAX;
	double ultimo = glfwGetTime(); //devuelve el tiempo en segundos
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		if (actual - ultimo >= 2.0) // cambia cada 2 segundo
		{
			// Generar nuevos colores RGB aleatorios
			uno = (float)rand() / RAND_MAX;
			dos = (float)rand() / RAND_MAX;
			tres = (float)rand() / RAND_MAX;
			ultimo = actual;
		}

		glClearColor(uno, dos, tres, 1.0f);
